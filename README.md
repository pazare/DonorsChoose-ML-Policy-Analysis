...
Why a bottom-10% list rather than the raw threshold: at the 0.68 threshold the model flags 78,523 of the 185,103 test projects (42%) as at-risk — with precision 0.49 and recall 0.68 for the unfunded class at the model's 0.757 test ROC AUC — far more than a small review team can act on, so the recommendation ranks projects by predicted funding probability and hands reviewers only the lowest 10% (about 18,500 projects on a test-cohort-sized batch); the notebook does not separately report precision or recall at that decile cut.
